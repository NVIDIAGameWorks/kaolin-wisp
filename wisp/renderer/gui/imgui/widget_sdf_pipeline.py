    ...
    def get_acceleration_structure_widget(self, acceleration_structure_name):
        if self.accel_widget is None:
            if acceleration_structure_name in self.names_to_widgets:
                self.accel_widget = self.names_to_widgets[acceleration_structure_name]()
        return self.accel_widget

    def paint(self, state: WispState, sdf: NeuralSDF = None, *args, **kwargs):
        if sdf is not None:
            if sdf.grid is not None:
                ...
                if imgui.tree_node("Grid Details", imgui.TREE_NODE_DEFAULT_OPEN):
                    ...
                    acceleration_structure = sdf.grid
            ...
            properties = {}
            if sdf.pos_embed_dim is not None:
                properties["Positional enc. dims"] = sdf.pos_embed_dim
            if len(properties) > 0:
