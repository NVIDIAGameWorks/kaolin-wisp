...
class SeparatedDecoderNeuralRadianceField(BaseNeuralField):
    ...
    def init_decoder(self):
        ...
        self.density_input_dim = self.effective_feature_dim

        if self.position_input:
            self.density_input_dim += self.pos_embed_dim

        self.rgb_input_dim = self.density_input_dim + self.view_embed_dim

        self.density_decoder = BasicDecoder(self.density_input_dim, 1, get_activation_class(self.activation_type), True,
                                    layer=get_layer_class(self.layer_type), num_layers=self.num_layers,
                                    hidden_dim=self.hidden_dim, skip=[])

        self.rgb_decoder = BasicDecoder(self.rgb_input_dim, 3, get_activation_class(self.activation_type), True,
                                    layer=get_layer_class(self.layer_type), num_layers=self.num_layers,
                                    hidden_dim=self.hidden_dim, skip=[])
    ...
    def rgba(self, coords, ray_d, pidx=None, lod_idx=None):
        ...
        if self.position_input:
            ...
            fdir_alpha = torch.cat([feats,
                self.pos_embedder(coords.reshape(-1, 3))], dim=-1)
        else: 
            fdir_rgb = torch.cat([feats,
                self.view_embedder(-ray_d)[:,None].repeat(1, num_samples, 1).view(-1, self.view_embed_dim)], dim=-1)
            fdir_alpha = feats
        ...
        rgb = self.rgb_decoder(fdir_rgb)
        alpha = self.density_decoder(fdir_alpha)
