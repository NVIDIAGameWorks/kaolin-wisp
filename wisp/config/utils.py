...
""" A module for the public api of wisp's config system.
    A common use of the various methods is as follows:
    ...
    # Read args by priority: cli args > config yaml > config defaults
    # Example CLI:
    ...
    #       lr: 1e-3
    #       eps: 1e-8
    # cfg is an AppConfig instance with filled values
    cfg = parse_config(AppConfig, yaml='path_here', create_if_not_exists=True)
    
    grid = instantiate(cfg.grid)                # Build grid instance directly from config
    nerf = instantiate(cfg.nerf, grid=grid)     # Build nerf instance from config + additional objects needed
    optimizer = instantiate(cfg.optimizer)      # Build optimizer from config...
    ```
"""
